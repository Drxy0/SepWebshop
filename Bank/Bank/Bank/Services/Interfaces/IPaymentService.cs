...
{
    Task<InitializePaymentServiceResult> InitializePayment(PaymentInitRequest request, Guid pspId, string signature, DateTime timestamp, bool isQrPayment);

    Task<string> ExecuteCardPayment(Guid paymentRequestId, PayByCardRequest request);

    Task<GetPaymentRequestResponse> GetPaymentRequest(Guid paymentRequestId);
